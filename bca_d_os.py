...
root = Tk()
root.title("BCA-D (Operating System)")
...
home.configure(font=('arial', 11, 'bold'), bg='green', fg='white')
...
ttk.Label(root, text="Enrollment No. ", background="black", foreground="white", font=fontStyle_t).grid(row=7, column=0, sticky="nsew")
ttk.Label(root, text="Name", background="black", foreground="white", font=fontStyle_t).grid(row=7, column=1, sticky="nsew")
ttk.Label(root, text="Roll No.", background="black", foreground="white", font=fontStyle_t).grid(row=7, column=2, sticky="nsew")
ttk.Label(root, text="Status", background="black", foreground="white", font=fontStyle_t).grid(row=7, column=3, sticky="nsew")
ttk.Label(root, text="", background="black", foreground="white").grid(row=7, column=4, sticky="nsew")
...
query = "SELECT * FROM bca_d"
mycursor.execute(query)
records = mycursor.fetchall()
var = []
for i, attendance_gui in enumerate(records):
    sEnroll, sName, sRoll = attendance_gui
...
def submit_attendance():
    ...
    for i, attendance_gui in enumerate(records):
        sEnroll, sName, sRoll = attendance_gui
        query = "INSERT INTO bca_d_os (date, sEnroll, sName, sRoll, status) VALUES (%s, %s, %s, %s, %s)"
